...
public class QtiRilInterface implements BaseRilInterface {
    ...
    public void sendPhoneStatus(int isReady, int phoneId) {
        ...
        AsyncResult sendQcRilHookMsg = this.mQcRilHook.sendQcRilHookMsg((int) IQcRilHook.QCRIL_EVT_HOOK_SET_ATEL_UI_STATUS, requestData, phoneId);
    }
    ...
    public void updateAdnRecord(QtiSimPhoneBookAdnRecord adnRecordInfo, Message callbackMsg, int phoneId) {
        ...
        int dataSize = 10 + (TextUtils.isEmpty(name) ? 0 : name.getBytes().length + 1) + (TextUtils.isEmpty(number) ? 0 : number.getBytes().length + 1);
        ...
        while (true) {
            ...
            reqBuffer.putShort((short) adnRecordInfo.getEmails()[i3].getBytes().length);
            try {
                reqBuffer.put(adnRecordInfo.getEmails()[i3].getBytes("UTF-8"));
                reqBuffer.put(0);
                i2 = i3 + 1;
        ...
        while (true) {
            int j3 = j2;
            if (j3 < numAdNumbers) {
                reqBuffer.putShort((short) adnRecordInfo.getAdNumbers()[j3].getBytes().length);
                try {
                    reqBuffer.put(QtiSimPhoneBookAdnRecord.ConvertToRecordNumber(adnRecordInfo.getAdNumbers()[j3]).getBytes("UTF-8"));
                    reqBuffer.put(0);
                    j2 = j3 + 1;
            ...
            if (numEmails > 0) {
                ...
                while (true) {
                    ...
                    if (emailLength > 0) {
                        ...
                        byteBuf.get(email);
                        try {
                            AdnRecordsInfoGroup[i].mEmails[j2] = new String(email, "UTF-8");
                        } catch (UnsupportedEncodingException e3) {
                            loge("Unsupport UTF-8 to parse email");
                        }
                    }
            ...
            if (numAnrs > 0) {
                ...
                while (true) {
                    ...
                    if (anrLength > 0) {
                        ...
                        byteBuf.get(anr);
                        try {
                            AdnRecordsInfoGroup[i].mAdNumbers[k3] = QtiSimPhoneBookAdnRecord.ConvertToPhoneNumber(new String(anr, "UTF-8"));
                        } catch (UnsupportedEncodingException e4) {
                            loge("Unsupport UTF-8 to parse anr");
                        }
                    }

...
public class QtiPrimaryCardController extends Handler {
    ...
    private void handleSetNwModeDone(Message msg) {
        ...
        if (ar.exception != null) {
            ...
            this.mRetryCount = i2;
            if (i2 > this.mRetryArray.length || requestType != 1000) {
                resetSetNwModeFailureCount();
                while (true) {
                    int i3 = i;
                    if (i3 >= QtiPrimaryCardUtils.PHONE_COUNT) {
                        break;
                    }
                    this.mPhone[i3].getPreferredNetworkType(obtainMessage(4, Integer.valueOf(i3)));
                    i = i3 + 1;
                }
                sendSetPrimaryCardResult(2);
            } else {
                int delay = this.mRetryArray[this.mRetryCount - 1] * 1000;
    ...
    private void handleOnSetPrimaryCardDone(Message msg) {
        ...
        if (ar.exception == null) {
            ...
            if (this.mContext.getResources().getBoolean(17957010)) {
                if (numCmccCards == 0) {
                    enableUserSelection = DBG;
                }
